...
sf::Packet & sf::operator>>(sf::Packet & packetReceived, ServerMessage & serverMessage)
{
	...
	serverMessage.faction = static_cast<FactionName>(faction);
	...
	for (int i = 0; i < spawnPositionsSize; ++i)
	{
		...
		serverMessage.spawnPositions.emplace_back(static_cast<FactionName>(factionName), x, y);
	}
	...
	for (int i = 0; i < existingFactionsSize; ++i)
	{
		...
		serverMessage.existingFactions.emplace_back(static_cast<FactionName>(existingFactionName), std::move(existingShips), AIControlled);
	}
